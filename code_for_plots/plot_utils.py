...
def plot_2d_hist(x, y, sizes=None, marksize=1, filename=None, xlabel=None, ylabel=None):
	

	xedges = np.linspace(sizes[0], sizes[1], 1000)
	yedges = np.linspace(sizes[2], sizes[3], 1000)
	...
	im.set_data(xcenters, ycenters, -H) # minus sign intended to flip greyscale
	ax.images.append(im)

	plt.axis(sizes)
	#plt.tight_layout()
	if filename:
		plt.savefig(DIRECTORY + filename+"2d.eps")
...
def plot(x, y, sizes=None, marksize=1, filename=None, xlabel=None, ylabel=None):
	plt.plot(x, y, 'ks', ms=500*marksize, fillstyle = 'none')
	ax = plt.gca()
	if xlabel:
		ax.set_xlabel(xlabel, fontsize=12)
		#ax.xaxis.set_label_coords(0.92,-0.06)
...
def error_plot(exact, approx, sizes=None, step=0.05, filename=None, title=None, barrier=1, relflag=True, absflag=True, Xlabel=None, Ylabel=None):
	err = exact - approx
	if relflag:
		err /= exact
	if absflag:
	...
	err[err<-barrier] = -(barrier+step)
	bins = int(1/step) + 1
	plt.hist(err, bins, hatch='/', edgecolor='k', fill=False)
	ax = plt.gca()
	...
	if sizes:
		plt.axis(sizes)
	if title:
		ax.annotate(title + "RMS(-1,1) ={:0.3f}".format(np.sqrt(np.mean((err)**2))), xy=(0.65, 0.85), xycoords='axes fraction', fontsize=14)
...
def smear_plot(exact, approx, sizes=None, step=0.05, filename=None, title=None, Xlabel=None, Ylabel=None, relflag=True, absflag=True):
	...
	ax = plt.gca()
	if title:
		ax.annotate(title + "\nRMS={:0.3f}".format(np.sqrt(np.mean((exact - approx)**2))), xy=(0.6, 0.8), xycoords='axes fraction', fontsize=12)
	ax.set_xlabel(Xlabel, fontsize=12)
	ax.set_ylabel(Ylabel, fontsize=12)
	if sizes:
		plt.axis(sizes)
...
def smear_plot2(exact, approx, sizes=None, step=0.05, filename=None, title=None, Xlabel=None, Ylabel=None, relflag=True, absflag=True, label1 = "", label2 = ""):
	delta1 = exact[0] - approx[0]
	delta2 = exact[1] - approx[1]

	for i in range(len(delta1)):
		while abs(delta1[i]) > np.pi:
			delta1[i] = delta1[i] - np.sign(delta1[i]) * 2 * np.pi


	for i in range(len(delta2)):
		while abs(delta2[i]) > np.pi:
			delta2[i] = delta2[i] - np.sign(delta2[i]) * 2 * np.pi 
	...
	n, bins, patches = plt.hist([delta1, delta2], bins, label=[label1 + " RMS={:0.3f}".format(np.sqrt(np.mean((exact[0] - approx[0])**2))), label2 + " RMS={:0.3f}".format(np.sqrt(np.mean((exact[1] - approx[1])**2)))])
	print(bins)
	hatches = ['//', '']
	...
	ax = plt.gca()
	if title:
		ax.annotate(title, xy=(0.4, 0.9), xycoords='axes fraction', fontsize=12)
	ax.set_xlabel(Xlabel, fontsize=12)
	ax.set_ylabel(Ylabel, fontsize=12)
	if sizes:
		plt.axis(sizes)
...
def smear_plot_multi(exact, approx, sizes=None, step=0.05, filename=None, title=None, Xlabel=None, Ylabel=None, relflag=True, absflag=True, labels = []):
	
	delta = []
	for i in range(len(exact)):
		delta1 = exact[i] - approx[i]
		for j in range(len(delta1)):
			while abs(delta1[j]) > np.pi:
				delta1[j] = delta1[j] - np.sign(delta1[j]) * 2 * np.pi
		delta.append(delta1)
		labels[i] = labels[i] + " RMS={:0.3f}, mean={:1.3f} ".format(np.sqrt(np.mean((exact[i] - approx[i])**2)), np.mean(exact[i] - approx[i]))
	bins = int(1/step) + 1
	plt.hist(delta, bins, label=labels)
	ax = plt.gca()
	if title:
		ax.annotate(title, xy=(0.4, 0.9), xycoords='axes fraction', fontsize=12)
	ax.set_xlabel(Xlabel, fontsize=12)
...
data = np.load("/home/kacper/doktorat/FAIS/Higgs_CP_state/HiggsCP_data/" + "a1rho_raw.data.npy")
...
tab = [data[:, 5 * i:5 * i + 4] for i in range(7)]
tab = np.array(tab)
tab2 = tab[:,:2,:]
...
pb_tau1_h = boost_and_rotate(p_tau1_h, PHI, THETA, p_a1_rho)
pb_tau2_h = boost_and_rotate(p_tau2_h, PHI, THETA, p_a1_rho)
...
'''
ve_alpha1_cms = pb_tau1_h.z / (pb_tau1_h.z + pb_tau1_nu.z)
ve_alpha2_cms = pb_tau2_h.z / (pb_tau2_h.z + pb_tau2_nu.z)
ve_alpha1_lab = p_tau1_h.length / (p_tau1_h.length + p_tau1_nu.length)
ve_alpha2_lab = p_tau2_h.length / (p_tau2_h.length + p_tau2_nu.length)

ve_x1_cms = 1 / (1 + ve_alpha1_cms)
ve_x2_cms = 1 / (1 + ve_alpha2_cms)
ve_x1_lab = 1 / (1 + ve_alpha1_lab)
ve_x2_lab = 1 / (1 + ve_alpha2_lab)
'''
...
'''plot(ve_x1_lab, ve_x1_cms, [0, 1.0, 0, 1.0], 0.01, filename="x1", xlabel=r"$x_1 \, (lab)$", ylabel=r"$x_1 \, (a1-rho)$")
plot(ve_x2_lab, ve_x2_cms, [0, 1.0, 0, 1.0], 0.01, filename="x2", xlabel=r"$x_2 \, (lab)$", ylabel=r"$x_2 \, (a1-rho)$")
