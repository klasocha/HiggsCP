...
def read_raw_all(kind, args):
    ...
    for letter in ["a", "b", "c", "d", "e", "f", "g", "h", "i", "k"][:args.DATASETS]:
        name = os.path.join(data_path, 
            "pythia.Z_65_155.a1rho.1M.%s.outTUPLE_labFrame" % (letter))
        print letter, name
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Prepare a1rhoZ data')
    ...
    np.save(os.path.join(data_path, "Z_65_155.a1rho_raw.data.npy"), data_a)
    np.save(os.path.join(data_path, "Z_65_155.a1rho_raw.w_a.npy"), weights_a)
    np.save(os.path.join(data_path, "Z_65_155.a1rho_raw.perm.npy"), perm)
