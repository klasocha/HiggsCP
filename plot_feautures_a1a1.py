...
DIRECTORY = "./control_plots/a1a1/"

def get_tau1(p):
	tau1_nu = p[0]                            

	tau1_h = sum(p[1:4])				# pi-, pi-, pi0

	return tau1_nu, tau1_h
...
data = np.load("../HiggsCP_data/" + "a1a1_raw.data.npy")
...
tab = [data[:, 5 * i:5 * i + 4] for i in range(8)]
tab = np.array(tab)

p = [Particle(i) for i in tab]

p_tau1_nu, p_tau1_h = get_tau1(p)

p_tau2_nu, p_tau2_h = get_tau2(p)

#------------------------------------------------------------
p_a1_a1 = p_tau1_h + p_tau2_h

PHI, THETA = calc_angles(p_tau1_h, p_a1_a1)

pb_tau1_h = boost_and_rotate(p_tau1_h, PHI, THETA, p_a1_a1)
pb_tau2_h = boost_and_rotate(p_tau2_h, PHI, THETA, p_a1_a1)
...
plot(ve_x1_lab, ve_x1_cms, [0, 1.0, 0, 1.0], 0.01, filename="x1", xlabel=r"$x_1 \, (lab)$", ylabel=r"$x_1 \, (a1-a1)$")
plot(ve_x2_lab, ve_x2_cms, [0, 1.0, 0, 1.0], 0.01, filename="x2", xlabel=r"$x_2 \, (lab)$", ylabel=r"$x_2 \, (a1-a1)$")
