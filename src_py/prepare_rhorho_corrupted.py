...
def read_raw_all(kinds, args, letters):
    ...
    for i,letter in enumerate(letters):
        name = os.path.join(data_path, "pythia.H.rhorho.1M.%s.CPmix_%s.outTUPLE_labFrame" % (letter, kinds[i]))
        print letter, name
...
if __name__ == "__main__":
    ...
    np.save(os.path.join(data_path, "rhorho_raw.w_06.npy"), weights[:,3])
    np.save(os.path.join(data_path, "rhorho_raw.w_08.npy"), weights[:,4])
    np.save(os.path.join(data_path, "rhorho_raw.w_10.npy"), weights[:,5])
    np.save(os.path.join(data_path, "rhorho_raw.w_12.npy"), weights[:,6])
    np.save(os.path.join(data_path, "rhorho_raw.w_14.npy"), weights[:,7])
    np.save(os.path.join(data_path, "rhorho_raw.w_16.npy"), weights[:,8])
