...
from src_py.logging_utils import ResultStorage, Logging
...
def total_train(pathOUT, model, data, args, emodel=None, batch_size=128, epochs=25):
    if emodel is None:
        emodel = model
    results_storage = ResultStorage(pathOUT)
    train_accs = []
    valid_accs = []
    test_accs = []
    train_losses = []
    ...
    logger.info("model = {}".format(model.tloss))
    ...
    for i in range(epochs):
        ...
        if model.tloss == 'soft':
            train_acc, train_mse, train_l1_delta_w, train_l2_delta_w = evaluate(emodel, data.train, args, 100000,
                                                                                filtered=True)
            ...
            logger.info(msg_str_0)
            logger.info(msg_str_1)
            logger.info(msg_str_2)
            ...
            if valid_acc == np.max(valid_accs):
                ...
                # print preds_w
                results_storage.save('softmax_calc_w.npy', calc_w)
                results_storage.save('softmax_preds_w.npy', preds_w)
        ...
        if model.tloss == 'regr_popts':
            ...
            results_storage.save('valid_regr_calc_popts.npy', valid_calc_popts)
            results_storage.save('valid_regr_preds_popts.npy', valid_pred_popts)

            test_calc_popts, test_pred_popts = regr_popts_predictions(emodel, data.test, filtered=True)
            results_storage.save('test_regr_calc_popts.npy', test_calc_popts)
            results_storage.save('test_regr_preds_popts.npy', test_pred_popts)
    ...
    if model.tloss == 'soft':
        ...
        results_storage.save('train_losses.npy', train_losses)

        results_storage.save('train_accs.npy', train_accs)

        results_storage.save('valid_accs.npy', valid_accs)

        results_storage.save('test_accs.npy', test_accs)

        results_storage.save('train_L1_deltas.npy', train_L1_deltas)

        results_storage.save('valid_L1_deltas.npy', valid_L1_deltas)

        results_storage.save('test_L1_deltas.npy', test_L1_deltas)

        results_storage.save('train_L2_deltas.npy', train_L2_deltas)

        results_storage.save('valid_L2_deltas.npy', valid_L2_deltas)

        results_storage.save('test_L2_deltas.npy', test_L2_deltas)
...
def calculate_classification_metrics(pred_w, calc_w, args):
    ...
    calc_pred_argmaxs_distances = calculate_deltas_unsigned(pred_arg_maxs, calc_arg_maxs, num_classes)
...
def test_roc_auc(preds_w, calc_w):
    ...
    for i in range(0, num_classes):
        ...
        pred_roc_auc = calculate_roc_auc(preds_w, calc_w, 0, i)
        logger.info('{} oracle_roc_auc: {} roc_auc: {}'.format(i + 1, oracle_roc_auc, pred_roc_auc))
...
def evaluate(model, dataset, args, at_most=None, filtered=True):
    ...
    # ERW
    # control print
    logger.info("evaluate: calc_w {}".format(calc_w))
    logger.info("evaluate: pred_w {}".format(pred_w))
    ...
    mse = np.mean(calculate_deltas_signed(pred_arg_maxs, calc_arg_maxs, num_classes))
