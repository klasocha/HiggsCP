...
def draw_distribution(variable, output_name, args, labels=None, weights=None, 
    ...
    output_path = os.path.join(os.path.normpath(args.OUT), f"phistar_y1y2_for_{args.NUM_CLASSES}_classes")
    ...
    # Saving the plot
    output_path = os.path.join(os.path.normpath(output_path), f"{output_name}_distribution.{args.FORMAT}")
...
def draw(args):
    ...
    if args.HYPOTHESIS != "None":
        ...
        if args.USE_UNWEIGHTED_EVENTS:
            weights = read_np(os.path.join(os.path.normpath(args.IN), f"unwt_multiclass_{args.NUM_CLASSES}.npy"))[:, int(args.HYPOTHESIS)]
        else:
            weights = read_np(os.path.join(os.path.normpath(args.IN), f"weights_multiclass_{args.NUM_CLASSES}.npy"))[:, int(args.HYPOTHESIS)]
