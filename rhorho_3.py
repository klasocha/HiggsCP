import numpy as np
from particle_3 import Particle
from math_utils_3 import *
...
class RhoRhoEvent(object):
    def __init__(self, data, args, debug=True):
        ...
        if((np.square(va_tau1_nu_E) - np.square(va_tau1_nu_long) < 0).any):
            print(np.square(va_tau1_nu_E) - np.square(va_tau1_nu_long))
        #------------------------------------------------------------
        #
        ...
        #
        va_tau1_nu_trans = np.zeros(va_tau1_nu_long.shape)
        va_tau2_nu_trans = np.zeros(va_tau2_nu_long.shape)
        ...
        ve_tau1_nu_trans = np.zeros(ve_tau1_nu_long.shape)
        ve_tau2_nu_trans = np.zeros(ve_tau2_nu_long.shape)
        ...
        self.cols = np.concatenate(cols,axis =  1)
