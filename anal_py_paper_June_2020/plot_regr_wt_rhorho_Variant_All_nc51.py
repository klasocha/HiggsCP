...
pathIN  = "../temp_results/nn_rhorho_Variant-All_regr_weights_hits_c0s_Unweighted_False_NO_NUM_CLASSES_51_SIZE_1000_LAYERS_6/monit_npy/"
pathOUT = "figures/"
...
x = np.linspace(0, k2PI, 51)
x2 = np.linspace(0, k2PI, 52) - k2PI/102
plt.plot(x,calc_w[i], label='Generated')
plt.step(x2,np.append(preds_w[i][0],preds_w[i]), label='Regresion: wt')
plt.legend()
#plt.ylim([0.0, 0.1])
plt.xlabel(r'$\alpha^{CP}$ [rad]')
#plt.xticks(np.arange(1, 52, 5))
...
x2 = np.linspace(0, k2PI, 52) - k2PI/102
plt.plot(x,calc_w[i], label='Generated')
plt.step(x2,np.append(preds_w[i][0],preds_w[i]), label='Regresion: wt')#plt.ylim([0.0, 0.1])
plt.xlabel(r'$\alpha^{CP}$ [rad]')
#plt.xticks(np.arange(1, 52, 5))
plt.ylabel(r'$wt$')
#plt.title('Features list: Variant-All')
