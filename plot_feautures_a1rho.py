...
def approx_alpha_B_new(v_ETmiss_x, v_ETmiss_y, p_tau1_h, p_tau2_h):
	M_H = 125.0
	M_TAU = 1.77

	nom_r = M_H**2 / 2 - M_TAU**2
	denom_r = ([-1,-1,-1, 1]*p_tau1_h*p_tau2_h).sum(1)
	
	a = -p_tau2_h.x
	b = p_tau1_h.x + v_ETmiss_x - p_tau2_h.x
	c = p_tau1_h.x + v_ETmiss_x - nom_r/denom_r * p_tau1_h.x

	alpha2_1 = (b + np.sqrt(b**2-4*a*c))/(2*a)
	alpha2_2 = (b - np.sqrt(b**2-4*a*c))/(2*a)

	
	alpha1_1 = nom_r/(denom_r * (alpha2_1 + 1)) - 1
	alpha1_2 = nom_r/(denom_r * (alpha2_2 + 1)) - 1

	return alpha1_1, alpha2_1, alpha1_2, alpha2_2, nom_r/denom_r
...
data = np.load("../HiggsCP_data/" + "a1rho_raw.data.npy")
...
#va_alpha1_B_new, va_alpha2_B_new, va_alpha1_B_new2, va_alpha2_B_new2, right = approx_alpha_B_new(v_ETmiss_x, v_ETmiss_y, p_tau1_h, p_tau2_h)
...
#va_tau1_nu_long_B_new = va_alpha1_B_new * pb_tau1_h.z
