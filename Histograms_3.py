...
def fill_histograms(data_hist_H: np.ndarray,data_hist_Z: np.ndarray,weights: np.ndarray, weights_Z : np.ndarray) ->None:
    ...
    u = 0
    ranges = {"pT" : np.linspace(0,50,50), "tautau_inv" : np.linspace(40,160,50), "rhorho_inv" : np.linspace(0,150,50),
    ...
    for i in range(len(variables_H)):
        if (u < len(names) - 6):
            ...
            plt.savefig(format = "eps", fname="figs/" + names[u] + "_pT" + ".eps", bbox_inches='tight')
            plt.close('all')
        for j in range(variables_H[i].shape[1]):
            ...
            plt.savefig(format = "eps", fname = "figs/" + names[u] + str(j) + ".eps", bbox_inches='tight')
            plt.close('all')
        u += 1
