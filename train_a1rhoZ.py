...
def total_train(model, data, emodel=None, batch_size=128, epochs=25, noise_fraction=0.2, saver=None):
    ...
    for i in range(epochs):
        ...
        # saver.save(tf.get_default_session(), "checkpoints/a1rho_model_%.1f.ckpt" % noise_fraction, i)
    return aucs
...
def load_a1rho(args):
    ...
    print "Loading data"
    data_ab = read_np(os.path.join(data_path, "a1rho_raw.data.npy"))
    w_a = read_np(os.path.join(data_path, "a1rho_raw.w_a.npy"))
    w_b = read_np(os.path.join(data_path, "a1rho_raw.w_b.npy"))
    perm_ab = read_np(os.path.join(data_path, "a1rho_raw.perm.npy"))
    print "Read %d events" % data_ab.shape[0]

    data_c = read_np(os.path.join(data_path, "Z_65_155.a1rho_raw.data.npy"))
    w_c = read_np(os.path.join(data_path, "Z_65_155.a1rho_raw.w_a.npy"))
    perm_c = read_np(os.path.join(data_path, "Z_65_155.a1rho_raw.perm.npy"))
    print "Read %d C events" % data_c.shape[0]

    print "Processing data"
    event_ab = A1RhoEvent(data_ab, args)
    ...
    event_c = A1RhoEvent(data_c, args)
