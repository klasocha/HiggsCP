...
def read_raw_all(kind, args, letter):
    ...
    for number in ["1", "2", "3", "4"]:
        name = os.path.join(data_path, "pythia.H.a1rho.1M.corr.%s_%s.CPmix_%s.outTUPLE_labFrame" % (letter, number, kind))
...
if __name__ == "__main__":
    ...
    '''for letter in ["a", "b", "c", "d", "e", "f", "g", "h", "i", "k"][:args.DATASETS]:
        ...
        np.save(os.path.join(data_path, "a1rho_raw.data%s.npy" %(letter)), data_a)
        np.save(os.path.join(data_path, "a1rho_raw.w_a%s.npy" %(letter)), weights_a)
        np.save(os.path.join(data_path, "a1rho_raw.w_b%s.npy" %(letter)), weights_b)

    data_path = args.IN
    data_a = np.load(os.path.join(data_path, "a1rho_raw.dataa.npy"))    
    #data_a = []
    weights_a = []
    weights_b = []

    for letter in [ "b", "c", "d", "e", "f", "g", "h", "i", "k"][:args.DATASETS]:
        data_a = np.vstack((data_a, np.load(os.path.join(data_path, "a1rho_raw.data%s.npy" %(letter)))))
        #data_a = np.append(data_a, np.load(os.path.join(data_path, "a1rho_raw.data%s.npy" %(letter))))
    np.save(os.path.join(data_path, "a1rho_raw.data.npy"), np.reshape(data_a,(-1,35)))
    del data_a'''
    ...
    for letter in ["a", "b", "c", "d", "e", "f", "g", "h", "i", "k"][:args.DATASETS]:
        weights_a = np.append(weights_a, np.load(os.path.join(data_path, "a1rho_raw.w_a%s.npy" %(letter))))
        weights_b = np.append(weights_b, np.load(os.path.join(data_path, "a1rho_raw.w_b%s.npy" %(letter))))
